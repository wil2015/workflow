...
namespace AutorizacaoCompra\Service;

use AutorizacaoCompra\Repository\AutorizacaoCompraRepo;
use Shared\Documentos\Engine\DocumentoEngine;
use AutorizacaoCompra\Documentos\AutorizacaoDoc;
...
class AutorizacaoCompraService
{
    private AutorizacaoCompraRepo $repo;
    ...
    public function listarDocumentosGerados(int $idProcesso): array
    {
    ...
    public function gerarDocumentosOficiais(int $idProcesso, int $idUsuario): array
    {
        $this->repo->executarSnapshotDados($idProcesso, $idUsuario);

        $autorizacoes = $this->repo->buscarAutorizacoesGeradas($idProcesso);
        if (empty($autorizacoes)) throw new Exception("Nenhuma autorizacao gerada.");
    ...
    public function enviarEmailsEConcluir(int $idProcesso, int $idUsuario): array
    {
        $dsnSmtp = $_ENV['SMTP_DSN'] ?? 'smtp://usuario:senha@smtp.mailtrap.io:2525';
        $transport = Transport::fromDsn($dsnSmtp);
            ...
            if ($doc && file_exists($pathFisico)) {
                try {
                    $email = (new Email())
                        ...
                        ->to($dados['intnet'])
                        ->subject("Autorizacao de Compra #$idProcesso")
                        ->html("<p>Prezado fornecedor, segue autorizacao anexa.</p>")
