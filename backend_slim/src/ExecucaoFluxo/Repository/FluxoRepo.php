...
class FluxoRepo extends BaseRepository
{
    ...
    public function getInstanciaCompleta($id): ?array
    {
    ...
    {
        $sql = "INSERT IGNORE INTO processos_itens (id_processo_instancia, num_solicitacao, seq_solicitacao, quantidade) VALUES (:id, :num, :seq, :qtd)";
        $stmt = $this->pdo->prepare($sql);
    ...
    {
        $sql = "UPDATE processos_instancia SET data_esperada_da_cotacao = :dt_cot, data_esperada_do_recebimento = :dt_rec WHERE id = :id";
        $stmt = $this->pdo->prepare($sql);
